        ...
        private void wordSearch(string _text) {
            ...
            for (int i = sze - 1; i > 0; i--) {
                if (text[i].Length >= 1) {
                    ...
                    else if (text[i].Contains('♭')) text[i] = text[i].Replace("♭", " flat") + " " + search;
                    else if (text[i].Contains('♭')) text[i] = text[i].Replace("#", " sharp") + " " + search;
                    else search = text[i] + " " + search;
                }
                if (text[i].Length == 1) break;
            }

            // If string search ends in a period, comma, exclamation mark, or question mark, or space, remove it
            if (search[search.Length - 1] == '.' || search[search.Length - 1] == ',' || search[search.Length - 1] == '!' || search[search.Length - 1] == '?' || search[search.Length - 1] == ' ') search = search.Substring(0, search.Length - 1);
            ...
            // If the search is not in the unordered map, return "Sorry, I don't know that."
            if (!unorderedMap.ContainsKey(search)) resultText = "Sorry, I don't have the answer to that question. Try another one!";
            else {
                resultText = unorderedMap[search];
            }

            pearAnswer.ReceiveAnswer(resultText);
